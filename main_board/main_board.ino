...
void parseSensorMessage(String msg) {
  ...
  if (s2pos >= 0 && s2pos + 3 < msg.length()) {
    stage2LimitHit = (msg.charAt(s2pos + 3) == '1');
  }
  if (s3pos >= 0 && s3pos + 3 < msg.length()) {
    stage3LimitHit = (msg.charAt(s3pos + 3) == '1');
  }
  if (s4pos >= 0 && s4pos + 3 < msg.length()) {
...
//
// IMPORTANT:
// Based on your command definitions:
//
// Stage 2:
//   s2up   -> forward = true
//   s2down -> forward = false
//   So only DOWN should be blocked => block when forward == false
//
// Stage 3:
//   s3down -> forward = true
//   s3up   -> forward = false
//   So only DOWN should be blocked => block when forward == true
//
// Stage 5:
//   no limit mapped yet
// =========================
bool shouldStopMotor(int motorIndex, bool forward) {
  ...
  // Stage 5 has no limit switch mapped yet
  return false;
}
...
void handleCommand(String cmd) {
  ...
  // Stage 5
  // NOTE: no limit switch mapped yet
  if (cmd.startsWith("s5ccw ")) {
    int steps = cmd.substring(7).toInt();
    if (steps > 0) {
      bool ok = stepMotor(STAGE5, true, steps);
      printlnBoth(ok ? "Stage 5 down done" : "Stage 5 down aborted");
    } else {
      printlnBoth("Invalid step count");
    }
    return;
  ...
  if (cmd.startsWith("s5cw ")) {
    ...
    if (steps > 0) {
      ...
      printlnBoth(ok ? "Stage 5 up done" : "Stage 5 up aborted");
    } else {
      printlnBoth("Invalid step count");
    }
    return;
