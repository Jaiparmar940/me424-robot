...
function editSequenceItem(index) {
  ...
  if (item.type === 'pose') {
    ...
    if (segs.length >= 2 && segs[1] !== '') {
      const msv = parseInt(segs[1], 10);
      if (Number.isNaN(msv) || msv < 10) return alert('maxSps must be an integer ≥ 10.');
      item.maxSps = msv;
    } else {
      delete item.maxSps;
    }
    if (segs.length >= 3 && segs[2] !== '') {
      const rmv = parseInt(segs[2], 10);
      if (Number.isNaN(rmv) || rmv < 1) return alert('ramp must be an integer ≥ 1.');
      item.ramp = rmv;
    } else {
      delete item.ramp;
    }
